...
**Prerequisite**: file `02_LDAP_Filter_Syntax_Reference.md`. Every payload below assumes you've internalized why `&`/`|` are prefix operators and why there's no SQLi-style infix OR bypass.
...
The application searches for the username first (e.g., `(uid=username)`) to resolve it to a DN, then performs a **real LDAP bind** with that DN and the supplied password. Here, the password is never part of a filter — it's used in an actual authentication call, so password-side filter injection doesn't apply. **The username-lookup filter is still injectable**, and is most useful for enumeration (file 05) or for influencing *which* DN gets resolved if the filter logic can be tricked into matching an unintended entry — e.g., an account with a known/blank password, or a service account.
